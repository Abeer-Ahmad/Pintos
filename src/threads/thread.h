...
struct thread {
    ...
    pid_t pid;                          /* Equals tid but set only when created by user process */
    enum thread_status status;          /* Thread state. */
    char name[16];                      /* Name (for debugging purposes). */
    uint8_t *stack;                     /* Saved stack pointer. */

    /* Priority Donation */
    int priority;                       /* Donated priority. Equals thread original priority when no donation needed */
    int original_priority;              /* Original priority. Also holds new priorities after call to "thread_set_priority"
                                         * until donation is no longer needed */
    ...
    /* Alarm */
    int wake_up_time;                   /* The time by which that thread should have woken up -if had slept- */
...
void mlfq_update_load_avg(void);

int insert_into_fdt(struct file *f);

struct file *get_file(int fd);

void insert_into_children(pid_t child_pid);
